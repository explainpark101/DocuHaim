...
- `webfonts`: on write/export, collect primary `fontFamily` tokens from text/shape elements (unset → default `Paperozi`); resolve CSS from app built-ins (`builtinWebfontCss.ts` / `builtin-webfonts.css`) and vault user webfonts; store as `{ family, css, source? }[]`. Omit when empty. Hosts that render the cover should inject `webfonts[].css` (see `CoverSlide`).
