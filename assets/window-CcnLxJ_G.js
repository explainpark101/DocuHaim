import { aa as r, j as i } from "./index-C9Mh46Eg.js";
import { listen as C, once as k, emit as L, emitTo as A, TauriEvent as o } from "./event-CHA9lK__.js";
import { t as D } from "./image-BezyKV5D.js";
class v {
  constructor(...e) {
...
class w {
  ...
  toPhysical(e) {
    return this.position instanceof a ? this.position : this.position.toPhysical(e);
  }
  [r]() {
...
function W() {
  return new z(window.__TAURI_INTERNALS__.metadata.currentWindow.label, { skip: true });
}
async function d() {
  return i("plugin:window|get_all_windows").then((t) => t.map((e) => new z(e, { skip: true })));
}
const p = ["tauri://created", "tauri://error"];
class z {
  constructor(e, n = {}) {
    var l;
    this.label = e, this.listeners = /* @__PURE__ */ Object.create(null), (n == null ? void 0 : n.skip) || i("plugin:window|create", { options: { ...n, parent: typeof n.parent == "string" ? n.parent : (l = n.parent) === null || l === void 0 ? void 0 : l.label, label: e } }).then(async () => this.emit("tauri://created")).catch(async (u) => this.emit("tauri://error", u));
  }
  static async getByLabel(e) {
    var n;
    return (n = (await d()).find((l) => l.label === e)) !== null && n !== void 0 ? n : null;
  }
  static getCurrent() {
    return W();
  }
  ...
  static async getFocusedWindow() {
    for (const e of await d()) if (await e.isFocused()) return e;
    return null;
  }
  async listen(e, n) {
    ...
    } : C(e, n, { target: { kind: "Window", label: this.label } });
  }
  async once(e, n) {
    return this._handleTauriEvent(e, n) ? () => {
      const l = this.listeners[e];
      l.splice(l.indexOf(n), 1);
    } : k(e, n, { target: { kind: "Window", label: this.label } });
  }
  async emit(e, n) {
    if (p.includes(e)) {
      for (const l of this.listeners[e] || []) l({ event: e, id: -1, payload: n });
      return;
    }
    return L(e, n);
  }
  async emitTo(e, n, l) {
    if (p.includes(n)) {
      for (const u of this.listeners[n] || []) u({ event: n, id: -1, payload: l });
      return;
    }
    return A(e, n, l);
  }
  _handleTauriEvent(e, n) {
    return p.includes(e) ? (e in this.listeners ? this.listeners[e].push(n) : this.listeners[e] = [n], true) : false;
  }
  async scaleFactor() {
    return i("plugin:window|scale_factor", { label: this.label });
  }
  async innerPosition() {
    return i("plugin:window|inner_position", { label: this.label }).then((e) => new a(e));
  }
  async outerPosition() {
    return i("plugin:window|outer_position", { label: this.label }).then((e) => new a(e));
  }
  async innerSize() {
  ...
  async requestUserAttention(e) {
    let n = null;
    return e && (e === b.Critical ? n = { type: "Critical" } : n = { type: "Informational" }), i("plugin:window|request_user_attention", { label: this.label, value: n });
  }
  ...
  async setSizeConstraints(e) {
    ...
    return i("plugin:window|set_size_constraints", { label: this.label, value: { minWidth: n(e == null ? void 0 : e.minWidth), minHeight: n(e == null ? void 0 : e.minHeight), maxWidth: n(e == null ? void 0 : e.maxWidth), maxHeight: n(e == null ? void 0 : e.maxHeight) } });
  }
  async setPosition(e) {
    return i("plugin:window|set_position", { label: this.label, value: e instanceof w ? e : new w(e) });
  }
  ...
  async setCursorPosition(e) {
    return i("plugin:window|set_cursor_position", { label: this.label, value: e instanceof w ? e : new w(e) });
  }
  ...
  async onDragDropEvent(e) {
    const n = await this.listen(o.DRAG_ENTER, (s) => {
      e({ ...s, payload: { type: "enter", paths: s.payload.paths, position: new a(s.payload.position) } });
    }), l = await this.listen(o.DRAG_OVER, (s) => {
      e({ ...s, payload: { type: "over", position: new a(s.payload.position) } });
    }), u = await this.listen(o.DRAG_DROP, (s) => {
      e({ ...s, payload: { type: "drop", paths: s.payload.paths, position: new a(s.payload.position) } });
    }), O = await this.listen(o.DRAG_LEAVE, (s) => {
  ...
  async onFocusChanged(e) {
    const n = await this.listen(o.WINDOW_FOCUS, (u) => {
      e({ ...u, payload: true });
    }), l = await this.listen(o.WINDOW_BLUR, (u) => {
      e({ ...u, payload: false });
    });
    return () => {
      n(), l();
...
function y(t) {
  return t === null ? null : { name: t.name, scaleFactor: t.scaleFactor, position: new a(t.position), size: new h(t.size), workArea: { position: new a(t.workArea.position), size: new h(t.workArea.size) } };
}
async function I() {
  return i("plugin:window|current_monitor").then(y);
}
async function T() {
  return i("plugin:window|primary_monitor").then(y);
}
async function N(t, e) {
  return i("plugin:window|monitor_from_point", { x: t, y: e }).then(y);
}
async function M() {
  return i("plugin:window|available_monitors").then((t) => t.map(y));
}
async function R() {
  return i("plugin:window|cursor_position").then((t) => new a(t));
}
const H = Object.freeze(Object.defineProperty({ __proto__: null, CloseRequestedEvent: P, get Effect() {
  return _;
}, get EffectState() {
  return m;
}, LogicalPosition: f, LogicalSize: v, PhysicalPosition: a, PhysicalSize: h, get ProgressBarStatus() {
  return g;
}, get UserAttentionType() {
  return b;
}, Window: z, availableMonitors: M, currentMonitor: I, cursorPosition: R, getAllWindows: d, getCurrentWindow: W, monitorFromPoint: N, primaryMonitor: T }, Symbol.toStringTag, { value: "Module" }));
export {
  a as P,
  c as S,
  z as W,
