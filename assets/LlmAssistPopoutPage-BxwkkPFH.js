...
import { L as C, g as d, i as Y, a as y, p as Z, n as w, s as B, b as P, c as Q, d as m, e as X, f as H, r as J, l as A, h as b } from "./index-C0ZoiHxN.js";
import { S as K, X as $ } from "./vendor-lucide-DUUE-0mU.js";
...
typeof document < "u" && y(A());
const v = { selectedText: "", selectionRange: { from: 0, to: 0 }, attachedImages: [], instruction: "", systemPrompt: d(), requestOptions: { ...C }, result: "", resultViewMode: "text", loading: false, error: "", templates: [], selectedTemplateId: "", templateName: "", editingTemplateId: null, profiles: [], selectedProfileId: "", model: "", theme: A() };
function me() {
  const [a, R] = s.useState(v), [L, c] = s.useState(""), [N, p] = s.useState(() => d()), [k, u] = s.useState(() => ({ ...C })), [j, f] = s.useState(""), [E, h] = s.useState(""), [M, g] = s.useState("text"), [O, S] = s.useState(""), [x, I] = s.useState(""), [q, _] = s.useState(false), [r, D] = s.useState(false), i = s.useRef(false);
  s.useEffect(() => {
    Y().then(D);
  }, []), s.useEffect(() => {
    y(a.theme);
  ...
  const t = s.useCallback((e, o = {}) => {
    ...
    const o = { ...v, ...e };
    R(o), c(o.instruction), p(typeof o.systemPrompt == "string" ? o.systemPrompt : d()), u(w(o.requestOptions)), f(o.templateName), h(o.result), g(o.resultViewMode), S(o.model || ""), I(o.selectedProfileId || ""), _(true);
  }, []);
  s.useEffect(() => {
    let e = () => {
    }, o = false;
    return B((l) => {
      if (l.type === b.SYNC && l.state) {
        T(l.state);
        return;
      }
      l.type === b.PARENT_CLOSING && (r ? m() : window.close());
    }).then((l) => {
      if (o) {
        l();
        return;
    ...
    if (!i.current) {
      ...
      !window.opener || window.opener.closed || (i.current = true, P());
    }
  }, [r]), s.useEffect(() => {
    if (r) {
      const o = setInterval(() => {
  ...
  }, selectedText: a.selectedText, onRefreshSelection: () => t("refresh-selection"), attachedImages: a.attachedImages || [], onAddImages: async (e) => t("add-images", { images: e }), onRemoveImage: (e) => t("remove-image", { id: e }), instruction: L, onInstructionChange: V, systemPrompt: N, onSystemPromptChange: F, requestOptions: k, onRequestOptionsChange: z, result: E, onResultChange: U, resultViewMode: M, onResultViewModeChange: (e) => {
