import { r as m, j as u, a as X } from "./vendor-react-BFxggocB.js";
import { A as G, m as J } from "./vendor-motion-b8oTnHK_.js";
import { b as Q, d as Z, T as tt, e as et, f as nt, A as rt } from "./vendor-radix-ACO_3onn.js";
import { k as ot, ad as it, ae as st, af as at, ag as W, a8 as ct, ah as lt, ai as ut, aj as dt } from "./index-DPH8WKK6.js";
function ft(t) {
...
function Xt(t, e, n, o) {
  ...
  let g = ((_a = d[0]) == null ? void 0 : _a.y) ?? 0;
  const a = /* @__PURE__ */ new Map();
  for (const r of d) a.set(r.id, g), g += r.h + s;
  return t.map((r) => {
    const c = a.get(r.id);
...
function bt(t) {
  ...
  return { boxSizing: "border-box", display: "flex", flexDirection: "column", justifyContent: xt(t.textVAlign), width: "100%", height: "100%", margin: 0, padding: `${e}%`, overflow: "hidden" };
}
function vt(t, e) {
...
function wt(t, e) {
  ...
  const s = Math.max(48, Math.round(e.widthPx)), l = Math.max(32, Math.round(e.heightPx)), g = { ...o.meta ?? it(), width: "fit", boxWidth: `${s}px`, boxHeight: `${l}px` }, a = st(t, o, g, o.grid);
...
function Zt({ containerRef: t, rootEl: e = null }) {
  const [n, o] = m.useState(null), [s, l] = m.useState(null), d = m.useRef(null), g = m.useRef(0), a = m.useRef(null), r = m.useCallback(() => {
    d.current != null && (clearTimeout(d.current), d.current = null);
  }, []), c = m.useCallback(() => {
    r(), a.current && (g.current = Date.now()), a.current = null, o(null), l(null);
  }, [r]), E = m.useCallback((i, v) => {
    const h = { el: i, text: v }, S = () => {
      a.current = h, o(h), l(D(i));
    };
    if (r(), a.current) {
      S();
      return;
    }
    const b = Date.now() - g.current < Ct ? 0 : Tt;
    if (b === 0) {
      S();
      return;
    }
  ...
  m.useEffect(() => {
    ...
    return i.addEventListener("pointerover", v), i.addEventListener("pointerout", h), i.addEventListener("focusin", S), i.addEventListener("focusout", x), i.addEventListener("pointerdown", b), () => {
      r(), i.removeEventListener("pointerover", v), i.removeEventListener("pointerout", h), i.removeEventListener("focusin", S), i.removeEventListener("focusout", x), i.removeEventListener("pointerdown", b);
    };
...
function te({ containerRef: t, getMarkdown: e, setMarkdown: n, enabled: o = true }) {
  const [s, l] = m.useState(null), [d, g] = m.useState(null), a = m.useRef(null), r = m.useRef(false);
  a.current = s;
  const c = m.useCallback(() => {
    l(null), g(null), a.current = null;
  }, []);
  m.useEffect(() => {
    o || c();
  }, [c, o]), m.useEffect(() => {
    ...
    const v = () => {
      ...
      g({ left: h.left, top: h.top, width: h.width, height: h.height }), i = requestAnimationFrame(v);
    };
    ...
    const i = (v) => {
      ...
      const x = (_c = h.closest) == null ? void 0 : _c.call(h, "table");
      if (!x || !S.contains(x)) {
        c();
        return;
      }
      ...
      const b = yt(x, S);
      if (b < 0) return;
      const w = x.getBoundingClientRect(), p = { table: x, tableIndex: b, widthPx: Math.max(48, Math.round(w.width)), heightPx: Math.max(32, Math.round(w.height)) };
      a.current = p, l(p);
    };
  ...
  const E = m.useCallback((f, i) => {
    ...
    r.current = true;
    const h = f.clientX, S = f.clientY, x = v.widthPx, b = v.heightPx, w = b > 0 ? x / b : 1, p = f.pointerType === "touch";
    let y = false;
    const F = (P) => {
      const L = P.clientX - h, R = P.clientY - S;
      (Math.abs(L) > 1 || Math.abs(R) > 1) && (y = true);
      let T = x, C = b;
      if (i.includes("e") && (T = x + L), i.includes("w") && (T = x - L), i.includes("s") && (C = b + R), i.includes("n") && (C = b - R), T = Math.max(48, T), C = Math.max(32, C), p || P.shiftKey) {
        const U = Math.abs((T - x) / Math.max(1, x)), V = Math.abs((C - b) / Math.max(1, b));
      ...
      const P = a.current;
      if (!P || !y || P.widthPx === x && P.heightPx === b) return;
...
function Nt({ el: t, strictClip: e = false }) {
  const n = t.text ?? "";
  return u.jsx("div", { className: "h-full w-full", style: gt(t), "data-cover-shape": t.type, children: n ? u.jsx("div", { style: bt(t), children: u.jsx("div", { style: vt(t, { strictClip: e }), children: n }) }) : null });
}
function ee({ cover: t, getPresignedUrl: e, className: n = "", style: o, showFrameOutline: s = false, renderElements: l = true, children: d }) {
  const g = ft(t.layout), a = t.bg.color || "#ffffff";
  return u.jsxs("div", { className: `export-pdf-cover relative z-2 overflow-hidden bg-white text-gray-900 ${n}`, style: { width: "var(--print-page-width)", height: "var(--print-page-height)", backgroundColor: a, ...o }, "data-note-cover": "1", onContextMenu: (r) => {
    r.stopPropagation();
  }, children: [u.jsx(Rt, { cover: t }), t.bg.imagePath ? u.jsx(jt, { path: t.bg.imagePath, getPresignedUrl: e }) : null, u.jsxs("div", { className: `absolute top-0 bottom-0 ${s ? "outline outline-1 outline-dashed outline-blue-400/70" : ""}`, style: { left: `${g.leftPct}%`, width: `${g.widthPct}%` }, "data-cover-frame": "1", children: [l ? t.elements.map((r) => r.type === "text" ? u.jsx("div", { "data-cover-el": r.id, style: { ...j(r), ...pt(r) }, children: r.text }, r.id) : lt(r) ? u.jsx("div", { "data-cover-el": r.id, style: j(r), children: u.jsx(Nt, { el: r }) }, r.id) : u.jsx("div", { "data-cover-el": r.id, style: j(r), children: u.jsx(Ot, { path: r.path, getPresignedUrl: e }) }, r.id)) : null, d] })] });
}
...
function $(t, e) {
  ...
  if (!o) return false;
  const s = zt(o, e);
  return s ? (Dt(o) ? $t(s) : s.scrollIntoView({ block: "nearest", behavior: "smooth" }), true) : false;
}
function H(t, e) {
  var _a, _b;
...
function ne(t) {
  ...
  const e = (n) => {
    ...
    if (s instanceof HTMLElement && t.contains(s)) {
      ...
      if (!E) return;
      n.preventDefault(), n.stopPropagation(), $(E, c), O(c);
      return;
    }
    ...
    const d = l.getAttribute(Bt) || "", g = l.getAttribute("href") || "", a = d || (_t(g) ? g.slice(1) : "");
    if (!a) return;
