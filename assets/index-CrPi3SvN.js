import { E as f, A as p, ad as x, au as X, av as y, y as h, ac as G, ah as Z, t as Q, ae as S, ao as b, ag as j, al as V, ai as c } from "./vendor-md-editor-CyUZNHY0.js";
import "./vendor-react-SY5QCjFA.js";
const W = 1, k = 2, v = 3, w = 180, R = 4, _ = 181, z = 5, E = 182, U = 6;
function D(O) {
...
const OO = f.inputHandler.of((O, a, r, n) => n != "%" || a != r || O.state.doc.sliceString(a - 1, r + 1) != "{}" ? false : (O.dispatch(O.state.changeByRange(($) => ({ changes: { from: $.from, to: $.to, insert: "%%" }, range: p.cursor($.from + 1) })), { scrollIntoView: true, userEvent: "input.type" }), true));
function d(O) {
  return (a) => {
...
const $O = G.define({ name: "liquid", parser: H.configure({ props: [Z({ "cycle comment endcomment raw endraw echo increment decrement liquid in with as": Q.keyword, "empty forloop tablerowloop": Q.atom, "if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue": Q.controlKeyword, "assign capture endcapture": Q.definitionKeyword, contains: Q.operatorKeyword, "render include": Q.moduleKeyword, VariableName: Q.variableName, TagName: Q.tagName, FilterName: Q.function(Q.variableName), PropertyName: Q.propertyName, CompareOp: Q.compareOperator, AssignOp: Q.definitionOperator, LogicOp: Q.logicOperator, NumberLiteral: Q.number, StringLiteral: Q.string, BooleanLiteral: Q.bool, InlineComment: Q.lineComment, CommentText: Q.blockComment, "{% %} {{ }}": Q.brace, "[ ]": Q.bracket, "( )": Q.paren, ".": Q.derefOperator, ", .. : |": Q.punctuation }), S.add({ Tag: b({ closing: "%}" }), "UnlessDirective ForDirective TablerowDirective CaptureDirective": d(/^\s*(\{%-?\s*)?end\w/), IfDirective: d(/^\s*(\{%-?\s*)?(endif|else|elsif)\b/), CaseDirective: d(/^\s*(\{%-?\s*)?(endcase|when)\b/) }), j.add({ "UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment"(O) {
  let a = O.firstChild, r = O.lastChild;
  return !a || a.name != "Tag" ? null : { from: a.to, to: r.name == "EndTag" ? r.from : O.to };
} })] }), languageData: { commentTokens: { line: "#" }, indentOnInput: /^\s*{%-?\s*(?:end|elsif|else|when|)$/ } }), q = X();
function g(O) {
  return $O.configure({ wrap: y((a) => a.type.isTop ? { parser: O.parser, overlay: (r) => r.name == "Text" || r.name == "RawText" } : null) }, "liquid");
}
const aO = g(q.language);
function rO(O = {}) {
  let a = O.base || q, r = a.language == q.language ? aO : g(a.language);
  return new x(r, [a.support, r.data.of({ autocomplete: J(O) }), a.language.data.of({ closeBrackets: { brackets: ["{"] } }), OO]);
}
export {
