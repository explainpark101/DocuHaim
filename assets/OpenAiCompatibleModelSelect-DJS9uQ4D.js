...
import { g0 as q, g1 as X, g2 as H, e0 as V, eW as Z, g3 as L, eU as Q, eV as Y } from "./index-BGi2DbIx.js";
...
async function he(e, r = "") {
  const t = K(e), n = await W(`${t}/models`, { headers: z(r) });
  if (!n.ok) {
    const l = await J(n);
  ...
  for (const l of a) {
    const m = pe(l);
    !m || c.has(m) || (c.add(m), i.push({ id: m, displayName: fe(l, m) }));
  }
...
async function xe({ baseUrl: e, apiKey: r, modelId: t, messages: n }) {
  const s = await W(`${e}/chat/completions`, { method: "POST", headers: z(r), body: JSON.stringify({ model: t, messages: n, temperature: 0.4 }) });
  if (!s.ok) {
    const i = await J(s), c = new Error(U({ status: s.status, detail: i, modelId: t }));
...
async function Ne({ baseUrl: e, apiKey: r, model: t, instruction: n, selectedText: s, images: o }) {
  const a = K(e), i = (t || H()).trim(), c = (n || "").trim(), l = (s || "").trim(), m = Array.isArray(o) ? o.filter((y) => (y == null ? void 0 : y.mimeType) && (y == null ? void 0 : y.dataBase64)) : [];
...
function Me({ getGeminiApiKey: e, profileId: r = "gemini", value: t, onChange: n, autoLoad: s = false, className: o = "" }) {
  ...
  const [c, l] = p.useState(() => E(L, t)), [m, x] = p.useState(false), [y, h] = p.useState(""), f = p.useCallback(async ({ force: u = false } = {}) => {
    var _a;
    const g = a.current;
...
function Ae({ value: e, onChange: r, options: t, loading: n = false, placeholder: s = "", className: o = "", maxItems: a = 30 }) {
  const [i, c] = p.useState(false), l = (e || "").trim().toLowerCase(), m = p.useMemo(() => (l ? t.filter((w) => {
    const u = (w.id || "").toLowerCase(), g = (w.displayName || "").toLowerCase();
    return u.includes(l) || g.includes(l);
  }) : [...t]).slice(0, Math.max(1, a)), [t, l, a]), x = p.useCallback((f) => {
    r == null ? void 0 : r(f), c(false);
  }, [r]), y = !n && (l ? m.length === 0 : t.length === 0), h = l ? "\uC77C\uCE58\uD558\uB294 \uBAA8\uB378\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." : "\uBAA8\uB378 \uBAA9\uB85D\uC774 \uBE44\uC5B4 \uC788\uC2B5\uB2C8\uB2E4.";
  return d.jsxs(ee, { open: i, onOpenChange: c, modal: false, children: [d.jsx(te, { asChild: true, children: d.jsx("input", { type: "text", autoComplete: "off", spellCheck: false, value: e, onFocus: () => c(true), onClick: () => c(true), onChange: (f) => r == null ? void 0 : r(f.target.value), placeholder: s, "aria-label": "\uBAA8\uB378 ID", className: `min-w-0 flex-1 rounded border border-gray-300 bg-white px-2 py-1.5 text-sm dark:border-odp-borderStrong dark:bg-odp-bgSoft ${o}`.trim() }) }), d.jsx(re, { children: d.jsx(ne, { className: "z-100050 max-h-72 min-w-48 overflow-auto rounded-md border border-gray-200 bg-white p-1 shadow-lg dark:border-odp-borderStrong dark:bg-odp-bgSoft", side: "bottom", align: "start", sideOffset: 4, onCloseAutoFocus: (f) => f.preventDefault(), children: n ? d.jsx("div", { className: "px-2 py-1.5 text-xs text-gray-500 dark:text-odp-muted", children: "\uBAA9\uB85D\uC744 \uBD88\uB7EC\uC624\uB294 \uC911\u2026" }) : y ? d.jsx(R, { disabled: true, className: "cursor-default select-none rounded px-2 py-1.5 text-xs text-gray-500 dark:text-odp-muted", children: h }) : m.map((f) => d.jsx(R, { onSelect: (w) => {
    w.preventDefault(), x(f.id);
  }, className: "flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-left text-xs text-gray-800 outline-none data-highlighted:bg-gray-100 dark:text-odp-fg dark:data-highlighted:bg-odp-focusBg", children: f.displayName }, f.id)) }) })] });
}
function Le({ getBaseUrl: e, getApiKey: r, value: t, onChange: n, autoLoad: s = false, className: o = "" }) {
  const a = p.useRef(e), i = p.useRef(r);
  a.current = e, i.current = r;
  const [c, l] = p.useState([]), [m, x] = p.useState(false), [y, h] = p.useState(""), f = p.useCallback(async () => {
    var _a, _b;
