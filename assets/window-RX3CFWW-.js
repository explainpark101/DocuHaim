import { gq as a, dD as O, dC as i } from "./index-C76l_n0j.js";
import { listen as S, once as L, emit as P, emitTo as k, TauriEvent as o } from "./event-gRKUcz8i.js";
...
class d extends O {
  ...
  static async new(e, n, l) {
    return i("plugin:image|new", { rgba: b(e), width: n, height: l }).then((r) => new d(r));
  }
  static async fromBytes(e) {
    return i("plugin:image|from_bytes", { bytes: b(e) }).then((n) => new d(n));
  }
...
function T() {
  return new W(window.__TAURI_INTERNALS__.metadata.currentWindow.label, { skip: true });
}
async function y() {
  return i("plugin:window|get_all_windows").then((t) => t.map((e) => new W(e, { skip: true })));
}
...
class W {
  ...
  static async getFocusedWindow() {
    for (const e of await y()) if (await e.isFocused()) return e;
    return null;
  }
  async listen(e, n) {
    ...
    } : S(e, n, { target: { kind: "Window", label: this.label } });
  }
  async once(e, n) {
    return this._handleTauriEvent(e, n) ? () => {
      const l = this.listeners[e];
  ...
  async requestUserAttention(e) {
    let n = null;
    return e && (e === g.Critical ? n = { type: "Critical" } : n = { type: "Informational" }), i("plugin:window|request_user_attention", { label: this.label, value: n });
  }
  ...
  async setIcon(e) {
    return i("plugin:window|set_icon", { label: this.label, value: b(e) });
  }
  ...
  async setOverlayIcon(e) {
    return i("plugin:window|set_overlay_icon", { label: this.label, value: e ? b(e) : void 0 });
  }
  ...
  async onDragDropEvent(e) {
    ...
    }), C = await this.listen(o.DRAG_LEAVE, (s) => {
