import { r as s, j as a } from "./vendor-react-kfkzeLNk.js";
import { L as P, g as d, i as Y, a as b, p as Z, n as T, s as B, b as v, c as Q, d as m, e as X, f as H, r as J, l as R, h as C } from "./index-BnDr9Xzt.js";
import { S as K, X as $ } from "./vendor-lucide-C7LgkNTS.js";
...
const y = { selectedText: "", selectionRange: { from: 0, to: 0 }, attachedImages: [], instruction: "", systemPrompt: d(), requestOptions: { ...P }, result: "", resultViewMode: "text", loading: false, error: "", templates: [], selectedTemplateId: "", templateName: "", editingTemplateId: null, profiles: [], selectedProfileId: "", model: "", theme: R() };
function me() {
  const [n, A] = s.useState(y), [L, c] = s.useState(""), [N, p] = s.useState(() => d()), [k, u] = s.useState(() => ({ ...P })), [E, f] = s.useState(""), [j, h] = s.useState(""), [M, g] = s.useState("text"), [O, S] = s.useState(""), [x, w] = s.useState(""), [q, _] = s.useState(false), [r, D] = s.useState(false), i = s.useRef(false);
  ...
  const t = s.useCallback((e, o = {}) => {
    ...
    const o = { ...y, ...e };
    A(o), c(o.instruction), p(typeof o.systemPrompt == "string" ? o.systemPrompt : d()), u(T(o.requestOptions)), f(o.templateName), h(o.result), g(o.resultViewMode), S(o.model || ""), w(o.selectedProfileId || ""), _(true);
  }, []);
  s.useEffect(() => {
    let e = () => {
    }, o = false;
    return B((l) => {
      if (l.type === C.SYNC && l.state) {
        I(l.state);
        return;
      }
      l.type === C.PARENT_CLOSING && (r ? m() : window.close());
    }).then((l) => {
      if (o) {
        l();
        return;
    ...
    if (!i.current) {
      ...
      !window.opener || window.opener.closed || (i.current = true, v());
    }
  }, [r]), s.useEffect(() => {
    if (r) {
      const o = setInterval(() => {
  ...
  }, selectedText: n.selectedText, onRefreshSelection: () => t("refresh-selection"), attachedImages: n.attachedImages || [], onAddImages: async (e) => t("add-images", { images: e }), onRemoveImage: (e) => t("remove-image", { id: e }), instruction: L, onInstructionChange: F, systemPrompt: N, onSystemPromptChange: V, requestOptions: k, onRequestOptionsChange: z, result: j, onResultChange: U, resultViewMode: M, onResultViewModeChange: (e) => {
