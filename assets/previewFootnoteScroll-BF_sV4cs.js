import { r as E, j as T, a as zt } from "./vendor-react-kfkzeLNk.js";
import { A as Vt, m as Xt } from "./vendor-motion-9P87yVtW.js";
import { d as Yt, e as Kt, T as Gt, f as Ut, g as Jt, A as Qt } from "./vendor-radix-CyZ5tkEq.js";
import { f as Zt, ap as te, aq as ee, ar as ne, as as St, W as re, at as oe, au as ie } from "./index-siMg0SyX.js";
import { m as I } from "./vendor-md-editor-DQ2k84v8.js";
function se(t) {
...
function gn(t, e, n) {
  return st() ? pe(t, e) ? true : n(t) ? (xe(t, e), true) : false : false;
}
function be(t, e, n) {
  var _a, _b;
...
function q(t, e, n) {
  ...
  const s = be(r, o.from, o.kind), f = Te(e, o.kind)[s];
  if (!f) return null;
  const a = G(f);
  return Ce(a, i);
}
...
function kt(t, e, n) {
  ...
  if (!n.collapsed && r && o && r === o) {
    ...
    const h = pt(o), g = gt(o), p = u[Math.min(h, u.length - 1)], x = p.spans[g] ?? p.spans[p.spans.length - 1];
    l = c.lineFrom + m.contentFrom, d = x ? p.lineFrom + x.contentTo : c.lineFrom + m.contentTo;
  } else {
...
function z(t, e, n) {
  ...
  for (const l of m) {
    ...
    const h = Ot(t, e, l).findIndex((x) => x.line0 === i);
    if (h < 0) continue;
    const g = Nt(l)[h];
    if (!g) continue;
    const p = Se(g, f);
...
function Le(t, e) {
  ...
  const o = (s) => {
    ...
    if (s instanceof HTMLBRElement) return n === 0 || (n -= 1, n === 0) ? xt(s) : null;
...
function Bt(t, e) {
  ...
  const a = (l, d) => {
    ...
    const { from: g, to: p } = Ht(t, e, h, h), x = t.state.doc.sliceString(g, p), b = et(d), y = ve(x, b, Math.max(0, Math.min(l, p) - g));
    return Le(d, y);
  }, c = a(r, u), m = a(o, f);
  if (!c || !m) return null;
  try {
...
function Ne(t, e) {
  ...
  if (r.collapsed) {
    const p = i || s, x = p ? et(p) : "", b = p ? V(p, r.startContainer, r.startOffset) : 0, y = X(m, x, b), C = a + Math.max(0, Math.min(y, m.length));
    return { from: C, to: C };
  }
  const l = n.toString();
  if (!l) return null;
  ...
  const h = bt(m, l, d) || bt(m, l, 0);
  if (h) return { from: a + h.from, to: a + h.to };
  if (i && i === s) {
    const p = et(i), x = V(i, r.startContainer, r.startOffset), b = V(i, r.endContainer, r.endOffset), y = Math.min(x, b), C = Math.max(x, b), w = X(m, p, y), O = X(m, p, C);
    ...
    if ((!o || o.height <= 0) && e.collapsed && e.startContainer instanceof Element) {
      ...
      if (c instanceof HTMLBRElement) {
        ...
        if (p) try {
          ...
          (b.height > 0 || b.width > 0) && (h = b.left, g = b.bottom);
        } catch {
        }
...
function wn({ containerRef: t, rootEl: e = null }) {
  const [n, r] = E.useState(null), [o, i] = E.useState(null), s = E.useRef(null), u = E.useRef(0), f = E.useRef(null), a = E.useCallback(() => {
    ...
    const b = Date.now() - u.current < He ? 0 : Fe;
    if (b === 0) {
      p();
      return;
    }
  ...
  E.useEffect(() => {
    ...
    return d.addEventListener("pointerover", h), d.addEventListener("pointerout", g), d.addEventListener("focusin", p), d.addEventListener("focusout", x), d.addEventListener("pointerdown", b), () => {
      a(), d.removeEventListener("pointerover", h), d.removeEventListener("pointerout", g), d.removeEventListener("focusin", p), d.removeEventListener("focusout", x), d.removeEventListener("pointerdown", b);
    };
...
function Sn({ containerRef: t, getMarkdown: e, setMarkdown: n, enabled: r = true }) {
  ...
  E.useEffect(() => {
    ...
    const d = (h) => {
      ...
      const x = (_c = g.closest) == null ? void 0 : _c.call(g, "table");
      if (!x || !p.contains(x)) {
        c();
        return;
      }
      ...
      const b = ge(x, p);
      if (b < 0) return;
      const y = x.getBoundingClientRect(), C = { table: x, tableIndex: b, widthPx: Math.max(48, Math.round(y.width)), heightPx: Math.max(32, Math.round(y.height)) };
      f.current = C, i(C);
    };
  ...
  const m = E.useCallback((l, d) => {
    ...
    a.current = true;
    const g = l.clientX, p = l.clientY, x = h.widthPx, b = h.heightPx, y = b > 0 ? x / b : 1, C = l.pointerType === "touch";
    let w = false;
    const O = (v) => {
      const P = v.clientX - g, W = v.clientY - p;
      (Math.abs(P) > 1 || Math.abs(W) > 1) && (w = true);
      let S = x, M = b;
      if (d.includes("e") && (S = x + P), d.includes("w") && (S = x - P), d.includes("s") && (M = b + W), d.includes("n") && (M = b - W), S = Math.max(48, S), M = Math.max(32, M), C || v.shiftKey) {
        const Wt = Math.abs((S - x) / Math.max(1, x)), qt = Math.abs((M - b) / Math.max(1, b));
      ...
      const v = f.current;
      if (!v || !w || v.widthPx === x && v.heightPx === b) return;
