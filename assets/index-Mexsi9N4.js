import { E as d, r as g, ad as v, au as m, av as u, G as x, ac as G, ah as b, t, ae as Z, ao as R, ag as h, al as k, ai as f, am as w } from "./vendor-md-editor-D4hOzNKK.js";
...
function S(O, i) {
  return O.split(" ").map((a) => ({ label: a, type: i }));
}
...
function A(O) {
  var i;
  let { state: a, pos: r } = O, e = x(a).resolveInner(r, -1).enterUnfinishedNodesBefore(r), P = ((i = e.childBefore(r)) === null || i === void 0 ? void 0 : i.name) || e.name;
...
function I(O = {}) {
  let i = O.tags ? O.tags.concat($) : $, a = O.variables ? O.variables.concat(l) : l, { properties: r } = O;
  return (e) => {
    var P;
    let Q = A(e);
    if (!Q) return null;
    let o = (P = Q.from) !== null && P !== void 0 ? P : Q.node ? Q.node.from : e.pos, n;
    return Q.type == "filter" ? n = N : Q.type == "tag" ? n = i : Q.type == "expr" ? n = a : n = r ? L(e.state, Q.target, e, r) : [], n.length ? { options: n, from: o, validFor: /^[\w\u00c0-\uffff]*$/ } : null;
  };
}
const H = d.inputHandler.of((O, i, a, r) => r != "%" || i != a || O.state.doc.sliceString(i - 1, a + 1) != "{}" ? false : (O.dispatch(O.state.changeByRange((e) => ({ changes: { from: e.from, to: e.to, insert: "%%" }, range: g.cursor(e.from + 1) })), { scrollIntoView: true, userEvent: "input.type" }), true));
function p(O) {
  return (i) => {
    let a = O.test(i.textAfter);
    return i.lineIndent(i.node.from) + (a ? 0 : i.unit);
  };
}
const B = G.define({ name: "jinja", parser: F.configure({ props: [b({ "TagName raw endraw filter endfilter as trans pluralize endtrans with endwith autoescape endautoescape": t.keyword, "required scoped recursive with without context ignore missing": t.modifier, self: t.self, "loop super": t.standard(t.variableName), "if elif else endif for endfor call endcall": t.controlKeyword, "block endblock set endset macro endmacro import from include": t.definitionKeyword, "Comment/...": t.blockComment, VariableName: t.variableName, Definition: t.definition(t.variableName), PropertyName: t.propertyName, FilterName: t.special(t.variableName), ArithOp: t.arithmeticOperator, AssignOp: t.definitionOperator, "not and or": t.logicOperator, CompareOp: t.compareOperator, "in is": t.operatorKeyword, "FilterOp ConcatOp": t.operator, StringLiteral: t.string, NumberLiteral: t.number, BooleanLiteral: t.bool, "{% %} {# #} {{ }} { }": t.brace, "( )": t.paren, ".": t.derefOperator, ": , .": t.punctuation }), Z.add({ Tag: R({ closing: "%}" }), "IfStatement ForStatement": p(/^\s*(\{%-?\s*)?(endif|endfor|else|elif)\b/), Statement: p(/^\s*(\{%-?\s*)?end\w/) }), h.add({ "Statement Comment"(O) {
  let i = O.firstChild, a = O.lastChild;
  return !i || i.name != "Tag" && i.name != "{#" ? null : { from: i.to, to: a.name == "EndTag" || a.name == "#}" ? a.from : O.to };
} })] }), languageData: { indentOnInput: /^\s*{%-?\s*(?:end|elif|else)$/ } }), s = m();
function c(O) {
  return B.configure({ wrap: u((i) => i.type.isTop ? { parser: O.parser, overlay: (a) => a.name == "Text" || a.name == "RawText" } : null) }, "jinja");
}
const J = c(s.language);
function OO(O = {}) {
  let i = O.base || s, a = i.language == s.language ? J : c(i.language);
  return new v(a, [i.support, a.data.of({ autocomplete: I(O) }), i.language.data.of({ closeBrackets: { brackets: ["{"] } }), H]);
}
export {
