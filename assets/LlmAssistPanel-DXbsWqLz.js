import { fm as ae, fO as j, eU as oe, fP as T, fQ as P, fR as se, dZ as de, Q as ne } from "./index-QBlKaQ6_.js";
import { r as a, j as e } from "./vendor-react-SY5QCjFA.js";
import { o as le } from "./vendor-md-editor-CyUZNHY0.js";
import { u as ie, M as pe } from "./useLazyMermaidRender-Du53WTD5.js";
import { O as ce, G as xe } from "./OpenAiCompatibleModelSelect-Z7C6xzHM.js";
...
function Ae(t) {
  const [s, d] = a.useState(() => j()), n = a.useMemo(() => {
    var _a;
    return ((_a = oe(t, s)) == null ? void 0 : _a.id) ?? "";
  }, [t, s]), o = a.useCallback((l) => {
    T(l), d(j());
  }, []), x = a.useCallback(() => {
    d(j());
  }, []);
  return a.useEffect(() => {
    const l = () => d(j());
    return window.addEventListener(P, l), () => window.removeEventListener(P, l);
  }, []), a.useEffect(() => {
    n && n !== s && (T(n), d(n));
  }, [s, n]), [n, o, x];
}
function ke({ profiles: t, value: s, onChange: d, className: n = "" }) {
  const o = a.useMemo(() => t.map((i) => ({ value: i.id, label: i.kind === "openai-compatible" ? `${i.name} \xB7 OpenAI \uD638\uD658` : `${i.name} \xB7 Gemini` })), [t]);
  if (!o.length) return e.jsx("p", { className: `text-[11px] text-amber-700 dark:text-amber-300 ${n}`.trim(), children: "\uC800\uC7A5\uB41C \uC81C\uACF5\uC790\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4. \uC124\uC815\uC5D0\uC11C \uCD94\uAC00\uD558\uC138\uC694." });
  const x = o[0], l = o.some((i) => i.value === s) ? s : (x == null ? void 0 : x.value) ?? "";
  return e.jsx(ae, { value: l, onValueChange: d, options: o, placeholder: "\uC81C\uACF5\uC790 \uC120\uD0DD", "aria-label": "AI \uC81C\uACF5\uC790", className: `w-full ${n}`.trim() });
}
const m = "s3haim-llm-assist", Ie = { READY: `${m}:ready`, SYNC: `${m}:sync`, ACTION: `${m}:action`, PARENT_CLOSING: `${m}:parent-closing` };
function Oe(t) {
  return !!(t && typeof t.type == "string" && t.type.startsWith(m));
}
function Te() {
  if (se()) {
...
const Pe = "s3haim-llm-assist", De = "popup=yes,width=480,height=820,menubar=no,toolbar=no,location=no,status=no,resizable=yes";
function Me(t, s, d = {}) {
  !t || t.closed || t.postMessage({ type: s, ...d }, window.location.origin);
}
const je = "llm-assist-result-preview";
function $e({ theme: t = "light", profiles: s = [], selectedProfileId: d = "", onSelectedProfileIdChange: n = () => {
}, selectedProfile: o = null, model: x = "", onModelChange: l, selectedText: i, onRefreshSelection: $, attachedImages: p = [], onAddImages: b, onRemoveImage: z, instruction: U, onInstructionChange: F, result: g, onResultChange: G, resultViewMode: u = "text", onResultViewModeChange: C, loading: S = false, error: E = "", templates: K = [], selectedTemplateId: H = "", onLoadTemplate: B, templateName: Q = "", onTemplateNameChange: W, editingTemplateId: X = null, onSaveTemplate: Y, onNewTemplate: Z, onDeleteTemplate: q, onRun: J, onApplyResult: V, remoteMode: ee = false, modelSelectAutoLoad: _ = true }) {
  const re = ee ? false : !g, N = a.useRef(null), R = a.useRef(null), f = a.useRef(null), h = a.useRef(p.length), [A, y] = a.useState(""), [v, k] = a.useState(false);
  ie(R, { layoutKey: `${t}|${g || ""}|${u}` }), a.useEffect(() => {
    h.current = p.length;
  }, [p.length]);
  const I = a.useCallback(async (r) => {
