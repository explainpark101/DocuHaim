import { j as b, g7 as P, g8 as q, g9 as O, ga as j, r as k } from "./index-DUogPMHz.js";
function X(a) {
...
async function V(a, t, e = {}) {
  const n = await X(a), r = document.createElement("canvas"), o = r.getContext("2d");
...
async function Z(a, t, e, n = {}) {
  const r = await X(a), o = Math.max(1, r.naturalWidth || r.width), c = Math.max(1, r.naturalHeight || r.height), s = o / Math.max(1, e.cellWidth), l = c / Math.max(1, e.cellHeight), i = (t.x - e.originX) * s, w = (t.y - e.originY) * l, d = Math.max(1, t.width * s), g = Math.max(1, t.height * l), f = Math.max(1, Math.round(d)), u = Math.max(1, Math.round(g)), y = f / d, m = u / g, h = document.createElement("canvas");
...
async function at(a, t) {
  if (F(t) || await z(a)) return { color: "#ffffff", transparentDefault: true };
...
async function rt(a, t, e = {}) {
  const n = Number.isFinite(e.padRatio) ? Math.max(0, e.padRatio) : 1, r = await C(a), o = Math.max(1, r.naturalWidth || r.width), c = Math.max(1, r.naturalHeight || r.height), s = 1 + 2 * n, l = Math.min(1, G / (Math.max(o, c) * Math.max(1, s))), i = Math.max(1, Math.round(o * l)), w = Math.max(1, Math.round(c * l)), d = Math.max(0, Math.round(i * n)), g = Math.max(0, Math.round(w * n)), f = i + d * 2, u = w + g * 2, y = document.createElement("canvas");
...
function L(a, t, e) {
  const n = j(t), r = a.getAttribute("src") || "", o = !!r && !r.startsWith(J);
