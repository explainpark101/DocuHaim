 ...
 * Run: bun scripts/convert-relative-to-alias.ts
 */
...
function extensionSwapCandidates(filePath: string): string[] {
...
function toAlias(fromFile: string, specifier: string): string | null {
...
function convertFile(filePath: string): boolean {
  ...
  const next = original.replace(IMPORT_RE, (match, specifier: string) => {
