...
# Arch/Omarchy gdk-pixbuf2 >= 2.42 uses built-in loaders. pkg-config still reports
# /usr/lib/gdk-pixbuf-2.0/2.10.0 but that directory is absent — skip copy then.
...
EOF
    ...
    echo "WARNING: gdk-pixbuf loaders dir missing ($gdk_pixbuf_binarydir); using built-in loaders"
fi
