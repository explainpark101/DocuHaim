...
export default function ExportPDFPage({
  ...
  const updatePreviewView = useCallback((partial: any) => {
    setPreviewView((prev) => {
  ...
  const handleStageZoomChange = useCallback((zoomPercent: any) => {
    updatePreviewView({ zoomPercent });
  }, [updatePreviewView]);

  const navigatePreviewToHeading = useCallback((headingId: any) => {
    if (!headingId) return;
  ...
  const handleCoverChange = useCallback((nextCover: any) => {
  ...
  const writeEditorHandoff = useCallback((editorContent: any, file = currentFileRef.current) => {
  ...
  const handleTocItemClick = useCallback((id: any) => {
  ...
  useEffect(() => {
    ...
    const isCoverContextMenu = (event: any) => {
  ...
  useEffect(() => {
    ...
    const onOutsidePointerDown = (event: any) => {
  ...
  const updatePrintLayout = useCallback((partial: any) => {
    setPrintLayout((prev) => {
      const next = { ...prev, ...partial };
      savePrintPageLayout(next);
      return next;
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
          <div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2">
            ...
            <PrintPageSizeSelect
              ...
              onValueChange={(pageSizeId: any) => updatePrintLayout({ pageSizeId })}
            />
            <PrintPreviewNavSelect
              value={effectiveNavigation}
              disabled={viewControlsLocked}
              onValueChange={(navigation: any) => {
            ...
            {effectivePages === 2 && !viewControlsLocked ? (
              <PrintPreviewFirstPageSingleSwitch
                ...
                onCheckedChange={(firstPageSingle: any) => {
            ...
            <PrintPreviewZoomControls
              ...
              onChange={(zoomPercent: any) => updatePreviewView({ zoomPercent })}
            />
      ...
      <PrintFontOptionsModal
        ...
        fonts={fonts}
        onFontsChange={(next: any) => setFonts(next)}
      />
      <WikiImageSizeModal
        key={
          wikiImageModalState
      ...
      <HaimTableBoxResizeLayer
        ...
        setMarkdown={(next: any) => setPreviewValue(next)}
      />
