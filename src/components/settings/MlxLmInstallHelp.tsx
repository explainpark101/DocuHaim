...
type InstallAction = 'uv' | 'mlx-lm' | 'huggingface-hub';

type MlxLmInstallHelpProps = {
...
function InstallHelpBody({
  ...
  const mlxInstalled = toolkit?.mlxLmInstalled === true;
  ...
  return (
    <>
      ...
      <p className="mb-2 text-[10px] text-gray-500 dark:text-odp-muted">
        ...
        mlx-lm / huggingface-hub CLI를 실행합니다.
      </p>
        ...
        <li>
          ...
          {uvAvailable && !mlxInstalled ? (
            <button
              ...
              className={ACTION_BTN_CLASS}
              disabled={busyAction != null}
              onClick={() => void runInstall('mlx-lm', installMlxLmTool)}
            >
              {busyAction === 'mlx-lm' ? <Loader2 size={12} className="animate-spin" /> : null}
              mlx-lm 설치
            </button>
          ) : null}
          ...
          {uvAvailable && mlxInstalled && hfInstalled ? (
            <p className="mt-1 text-[10px] text-emerald-700 dark:text-emerald-300">
              mlx-lm · huggingface-hub installed
            </p>
          ) : null}
        </li>
        <li>
...
export default function MlxLmInstallHelp({ toolkit, onRefresh }: MlxLmInstallHelpProps) {
