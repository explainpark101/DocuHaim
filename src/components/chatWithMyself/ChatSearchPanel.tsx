...
function SearchResultCard({
  ...
  const matchedReactionKeys = new Set(
    queryTokens.length > 0
      ? reactions
          .filter((reaction: any) => queryTokens.some((token: any) => fuzzyMatchText(reactionsToSearchText([reaction]), token),
    ),
          )
          .map((reaction: any) => reactionKey(reaction))
      : [],
  );
            ...
            {hasAttachments ? (
              ...
              >
                <ChatLinkedText
                  ...
                  onOpenViewPath={
                    onOpenNote
                      ? (path: any) => onOpenNote(path, result)
                      : undefined
                  }
                />
              </div>
...
export default function ChatSearchPanel({
  ...
  const isDateUnavailable = useMemo(
    () => (date: any) => !availableDaySet.has(date.toString()),
    [availableDaySet],
  );
  ...
  const groupFilterMeta = useMemo(() => {
    ...
    return {
      ...
      iconPath: (found as { iconPath?: string | null } | undefined)?.iconPath || null,
    };
        ...
        >
          ...
          <div className="flex flex-col gap-1.5 @[280px]:flex-row @[280px]:items-center">
            ...
            <div className="flex shrink-0 items-center gap-1.5 self-end @[280px]:self-auto">
              ...
              <button
                type="button"
                onClick={() => onFiltersOpenChange?.((v: any) => !v)}
                ...
                <div className="space-y-2 rounded-lg border border-dashed border-gray-200 bg-gray-50 p-2 dark:border-odp-borderSoft dark:bg-odp-bg/60">
                  ...
                  >
                    ...
                    <Switch.Root
                      ...
                      onCheckedChange={(next: any) => onNoReactionsOnlyChange?.(Boolean(next))
                      }
