...
export default function QuizStopwatchToolbar({
  stopwatch,
  onRequestStart,
}: QuizStopwatchToolbarProps) {
  const { displayMs, running, started, start, pause, resume, stop } = stopwatch;
  const handleStart = onRequestStart ?? start;
        ...
        <Tooltip.Root>
          <Tooltip.Trigger asChild>
            <Button type="button" variant="secondary" size="sm" onClick={handleStart}>
              <Timer size={14} />
              <span className="hidden md:inline">시험 스톱워치</span>
            </Button>
          </Tooltip.Trigger>
          <Tooltip.Portal>
