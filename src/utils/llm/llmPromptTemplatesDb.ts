...
import { createStorageBackend } from '@/utils/storage/createStorageBackend';
...
export const llmPromptTemplatesDb = new Dexie('s3haim-llm-prompts') as any;
...
let syncInFlight: any = null;
/** @type {string | null} */
let syncInFlightScope: any = null;
let syncGeneration = 0;
/** @type {string | null} */
let lastReadyScope: any = null;
...
function migratedKeyForScope(scope: any) {
 ...
 */
export function setLlmPromptTemplatesStore(payload: any) {
  if (!payload) return;
...
function hasMigratedIdbToRemote(scope: any) {
...
function markIdbMigratedToRemote(scope: any) {
...
function setIdbCacheScope(scope: any) {
  try {
 ...
 */
function normalizeTemplate(value: any) {
 ...
 * @returns {LlmPromptTemplate[] | null}
 */
function parseTemplatesPayload(parsed: any) {
...
function sortByUpdatedAtDesc(templates: any) {
 ...
 */
function mergeTemplates(remoteTemplates: any, idbTemplates: any) {
  const byId = new Map();
...
async function replaceIdbCache(templates: any) {
...
/** @param {LlmPromptTemplate[]} templates */
async function saveToRemote(templates: any) {
  const backend = getBackend();
  if (!backend?.isReady?.()) {
 ...
 * @returns {Promise<LlmPromptTemplate>}
 */
export async function saveLlmPromptTemplate(template: any) {
  const record = {
    ...template,
  ...
  const next = sortByUpdatedAtDesc([
    ...current.filter((t: any) => t.id !== record.id),
    record,
  ]);
...
/** @param {string} id */
export async function deleteLlmPromptTemplate(id: any) {
  const current = await listLlmPromptTemplates();
