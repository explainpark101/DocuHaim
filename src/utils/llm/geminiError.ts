...
export function isFreeTierBlockedModel(modelId: any) {
  return FREE_TIER_BLOCKED_MODELS.has(String(modelId || '').trim());
}

export function sanitizeGeminiModelId(modelId: any) {
 ...
 */
export function parseRetrySecondsFromGeminiError(detail: any) {
...
function hasZeroFreeTierLimit(detail: any) {
 ...
 * @returns {string}
 */
export function formatGeminiApiError({
  status,
  detail,
  modelId
}: any) {
