export type MlxLmFeasibility = 'ok' | 'tight' | 'unlikely' | 'unknown';
...
/** Sum HF sibling sizes (download footprint on disk). */
...
export function feasibilityLabel(feasibility: MlxLmFeasibility): string {
  switch (feasibility) {
    case 'ok':
      return '실행 가능';
    case 'tight':
...
export function feasibilityClassName(feasibility: MlxLmFeasibility): string {
  switch (feasibility) {
    case 'ok':
