...
function stripEditorOnlyNodes(root: any) {
  root.querySelectorAll(STRIP_SELECTORS.join(',')).forEach((node: any) => node.remove());
...
function copyDocumentStylesInto(doc: any) {
...
async function imageSrcToDataUrl(src: any) {
 ...
 */
function applyImageSrcReplacements(root: any, replacements: any) {
  if (!replacements) return;
  const lookup = replacements instanceof Map
    ? (key: any) => key ? replacements.get(key) : undefined
    : (key: any) => key ? replacements[key] : undefined;
...
async function inlineImages(root: any, skipKeys: any) {
...
function inlineComputedStyles(sourceRoot: any, targetRoot: any) {
...
function normalizeForPaste(root: any) {
  root.querySelectorAll('[contenteditable]').forEach((node: any) => node.removeAttribute('contenteditable'));
  root.querySelectorAll('[role="button"], [aria-label], [aria-hidden], [data-state]').forEach((node: any) => {
    node.removeAttribute('role');
    node.removeAttribute('aria-label');
    node.removeAttribute('aria-hidden');
    node.removeAttribute('data-state');
  });
  root.querySelectorAll('input[type="checkbox"]').forEach((input: any) => {
...
async function writeHtmlClipboard(html: any, text: any) {
