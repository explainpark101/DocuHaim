...
export const editorUndoHistoryDb = new Dexie('s3haim-editor-undo-history') as any;
 ...
 * @returns {string}
 */
export function getEditorUndoHistoryKey(storageType: any, path: any) {
 ...
 * @returns {string | null}
 */
export function getEditorUndoHistoryKeyFromFile(currentFile: any) {
 ...
 */
export async function getEditorUndoHistory(key: any) {
  if (!key) return null;
 ...
 * @returns {string[]}
 */
export function trimEditorUndoStack(stack: any) {
 ...
 */
export async function saveEditorUndoHistory({
  key,
  stack,
  index
}: any) {
 ...
 * @param {string} key
 */
export async function deleteEditorUndoHistory(key: any) {
  if (!key) return;
  await editorUndoHistoryDb.histories.delete(key);
}
 ...
 */
export function syncStackWithContent(stack: any, index: any, content: any) {
 ...
 */
export function pushEditorUndoCheckpoint(stack: any, index: any, content: any) {
  const current = content ?? '';
  const safeStack = Array.isArray(stack) && stack.length > 0 ? stack : [''];
