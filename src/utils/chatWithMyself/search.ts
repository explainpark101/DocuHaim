...
import { getCachedOg } from '@/utils/chatWithMyself/chatDb';
import { extractUrls, hashUrl } from '@/utils/chatWithMyself/og';
import { ogArchiveKey } from '@/utils/chatWithMyself/paths';
 ...
 */
export function fuzzyMatchText(haystack: any, needle: any) {
...
export function splitSearchTokens(query: any) {
  return String(query || '')
    .trim()
    .split(/\s+/)
    .filter(Boolean);
 ...
 */
export function fuzzyMatchTokensInHaystacks(haystacks: any, query: any) {
...
function fuzzySubsequence(haystack: any, needle: any) {
 ...
 */
export function ogDataToSearchText(data: any, url = '') {
 ...
 */
export async function loadOgSearchText(url: any, storage: any) {
  if (!url) return '';
 ...
 */
export async function loadMessageOgSearchText(msg: any, storage: any) {
 ...
 */
export function buildSearchResultDisplayText(body: any, query: any, ogSearchText = '') {
  const q = String(query || '').trim();
 ...
 */
export function highlightHtmlMatches(html: any, query: any) {
  const q = String(query || '').trim();
