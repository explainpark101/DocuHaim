...
} from '@/utils/chatWithMyself/editHistory';
import { cacheDay, getCachedDay, savePendingMessage } from '@/utils/chatWithMyself/chatDb';
import {
...
/** @param {string} name */
export function stableGroupIdFromName(name: any) {
  let h = 2166136261;
  const s = String(name || '');
...
export function resolveGroupLabel(groups: any, key: any) {
  const raw = String(key || '').trim();
...
/** @param {ChatGroup[]|string[]|unknown} groups @param {string} key */
export function resolveGroupId(groups: any, key: any) {
  const raw = String(key || '').trim();
  if (!raw || raw === SELF_GROUP) return SELF_GROUP;
 ...
 */
export function groupMatches(groups: any, messageGroup: any, filterKey: any) {
  return resolveGroupId(groups, messageGroup) === resolveGroupId(groups, filterKey);
}

function readMetaFromRaw(raw: any) {
  if (!raw) {
...
export async function readMeta(ctx: any) {
 ...
 * @returns {ChatGroup[]}
 */
export function sortGroupsKo(groups: any) {
...
/** @param {ChatGroup[]|string[]|unknown} groups */
export function groupNames(groups: any) {
...
export async function writeMeta(ctx: any, meta: any) {
  return mutateMeta(ctx, (current: any) => ({
...
export async function touchTimezone(ctx: any) {
 ...
 * @returns {Promise<ChatGroup[]>}
 */
export async function setGroupIcon(ctx: any, groupId: any, iconPath: any) {
 ...
 * @returns {Promise<ChatGroup[]>}
 */
export async function renameGroup(ctx: any, groupId: any, newName: any) {
  const id = String(groupId || '').trim();
...
/**
 * @returns {Promise<import('@/utils/chatWithMyself/format').ChatMessage[]>}
 */
export async function readDayMessages(ctx: any, dateStr: any) {
  const key = dayFileKey(dateStr);
 ...
 */
export async function readDayFileParsed(ctx: any, dateStr: any) {
  const key = dayFileKey(dateStr);
 ...
 */
export async function deleteChatMessage(ctx: any, dateStr: any, messageId: any) {
  if (!dateStr || !messageId) return false;
  let removed = false;
  await mutateDayFile(ctx, dateStr, (parsed: any) => {
    removed = false;
    const hadLive = parsed.messages.some((m: any) => m.id === messageId);
    ...
    return {
      messages: parsed.messages.filter((m: any) => m.id !== messageId),
 ...
 */
export async function writeMessageEditVersion(ctx: any, messageId: any, entry: any) {
...
export async function removeLegacyEditHistoryEntry(
  ...
  await mutateDayFile(ctx, dateStr, (parsed: any) => {
    ...
    const nextHist = prevHist.filter((e: any) => String(e?.at || '') !== String(at));
 ...
 * @returns {Promise<object | null>}
 */
export async function clearLegacyEditHistory(ctx: any, dateStr: any, messageId: any) {
  if (!dateStr || !messageId) return null;
  /** @type {object | null} */
  let updated = null;
  await mutateDayFile(ctx, dateStr, (parsed: any) => {
 ...
 */
export async function findMessageById(ctx: any, messageId: any, { maxDays = 90 } = {}) {
...
export async function readOgArchive(ctx: any, key: any) {
  const raw = await readText(ctx, key);
  if (!raw) return null;
  try {
...
export async function writeOgArchive(ctx: any, key: any, data: any) {
...
export function createOgStorageAdapters(ctx: any) {
  return {
    readArchive: (key: any) => readOgArchive(ctx, key),
    writeArchive: (key: any, data: any) => writeOgArchive(ctx, key, data),
  };
}

/** Rewrite entire day file (e.g. after merges) — preserves tombstones when provided */
export async function writeDayMessages(ctx: any, dateStr: any, messages: any, deletedAtById = {}) {
  await mutateDayFile(ctx, dateStr, (parsed: any) => mergeDayMessages(parsed, {
    messages: messages || [],
    deletedIds: Object.keys(deletedAtById || {}),
 ...
 */
export async function flushPendingMessages(ctx: any, db: any) {
 ...
 */
export async function unlinkChatNotesForDeletedPaths(ctx: any, scope: any) {
  const deleted = {
  ...
  for (const dateStr of keys) {
    ...
    await mutateDayFile(ctx, dateStr, (parsed: any) => {
      let dayUpdated = 0;
      const nextMessages = parsed.messages.map((msg: any) => {
...
export { mergeDayMessages, parseDayFile, serializeDeletedMarker } from '@/utils/chatWithMyself/format';
