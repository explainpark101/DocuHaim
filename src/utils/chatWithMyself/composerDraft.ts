...
export function composerDraftStorageKey(scope: any) {
  return scope ? `${COMPOSER_DRAFT_LS_KEY}:${scope}` : COMPOSER_DRAFT_LS_KEY;
}

function parseDraftMeta(raw: any) {
  if (!raw) return null;
  try {
...
export function readComposerDraftMeta(scope: any) {
...
export function writeComposerDraftMeta(scope: any, meta: any) {
...
/** @param {string} [scope] */
export async function clearComposerDraft(scope: any) {
 ...
 */
export async function loadComposerDraftImageQueue(scope: any, imageIds = []) {
  if (!scope) return [];
...
export function composerDraftHasContent(meta: any) {
