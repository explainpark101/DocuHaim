...
function getEditorApiFromRef(editorRef) {
  return editorRef?.current?.value ?? editorRef?.current ?? null;
}

function getMdEditorRootFromRef(editorRef, view = null) {
  if (view?.dom) {
 ...
 */
export function shouldAppendLlmResultAtDocEnd(editorRef, { view = null, from = 0, to = 0 } = {}) {
...
function buildAppendInsertAtDocEnd(docText, chunk) {
  if (!docText) return chunk;
  return docText.endsWith('\n') ? chunk : `\n${chunk}`;
}

function buildMarkdownAppendedAtDocEnd(docText, chunk) {
  if (!docText) return chunk;
