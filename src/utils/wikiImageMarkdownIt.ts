 ...
 */
function srcForStoragePath(path: any) {
 ...
 */
export function wikiImagePlugin(md: any) {
  // ![[path]] 또는 ![[path|size]] 형식 지원
  const WIKI_IMAGE_RE = /!\[\[([^[\]]+)\]\]/g;

  md.core.ruler.push('wiki-image', (state: any) => {
    let replaced = 0;
  ...
  md.core.ruler.after('wiki-image', 'wiki-image-caption-inline', (state: any) => {
    ...
    for (let i = 0; i < tokens.length; i += 1) {
      ...
      const hasContent = allCaptionTokens.some(
        (ct: any) => ct.type === 'text' && ct.content && ct.content.trim(),
      );
      if (!hasContent) {
        newTokens.push(t0);
        continue;
  ...
  md.core.ruler.after('wiki-image-caption-inline', 'wiki-image-caption', (state: any) => {
    const tokens = state.tokens;
  ...
  md.core.ruler.after('wiki-image-caption', 'markdown-image-size-attrs', (state: any) => {
    state.tokens.forEach((blockToken: any) => {
