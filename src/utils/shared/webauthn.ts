...
function randomBytes(length: any) {
  return crypto.getRandomValues(new Uint8Array(length));
}

function challengeBase64URL() {
  return bufferToBase64URLString(randomBytes(32) as unknown as ArrayBuffer);
}
...
export async function isWebAuthnPRFSupported() {
  ...
  if (!window.PublicKeyCredential || typeof (PublicKeyCredential as any).getClientCapabilities !== 'function') return false;
  try {
    const caps = await (PublicKeyCredential as any).getClientCapabilities('public-key');
 ...
 */
async function deriveWrapKey(prfOutput: any) {
...
async function deriveCredsKey(prfOutput: any) {
  const masterKey = await crypto.subtle.importKey(
    'raw',
    prfOutput,
    'HKDF',
...
async function decryptCredsWithPRFKeyLegacy(prfKeyOutput: any, blob: any) {
...
export async function createPasskeyWithPRF() {
  ...
  const userId = bufferToBase64URLString(randomBytes(16) as unknown as ArrayBuffer);
  ...
  const credential = await startRegistration({ optionsJSON: optionsJSON as any });
  const prfOut = (credential?.clientExtensionResults as any)?.prf;
 ...
 */
export async function getPasskeyPRFKey(credentialId: any, saltBase64: any) {
  const rpId = getRpId();
 ...
 */
export async function wrapPasswordWithPRFKey(prfKeyOutput: any, password: any) {
 ...
 */
export async function unwrapPasswordWithPRFKey(prfKeyOutput: any, wrapped: any) {
  const key = await deriveWrapKey(prfKeyOutput);
...
export function setStoredWebAuthn(data: any) {
  if (!data) {
 ...
 */
export async function enableWebAuthnUnlock(masterPassword: any) {
  if (isDesktopApp()) {
 ...
 */
export async function updateWebAuthnWrappedPassword(newMasterPassword: any) {
  if (getDesktopBiometryMarker()?.mode === 'password') {
 ...
 */
export async function saveCredsWithWebAuthn(creds: any) {
  if (isDesktopApp()) {
    if (!(await isDesktopBiometricAvailable())) {
      throw new Error('Biometric unlock is not available in this Tauri shell.');
    }
