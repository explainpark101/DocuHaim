const bufToBase64 = (buf: any) => btoa(Array.from(new Uint8Array(buf)).map((b) => String.fromCharCode(b)).join(''));

const base64ToBuf = (b64: any) => new Uint8Array(
  atob(b64)
    .split('')
    .map((c) => c.charCodeAt(0)),
);

const generateKey = async (password: any, salt: any) => {
...
export async function deriveKey(entropy: any, salt = new Uint8Array(16)) {
...
export async function encryptWithEntropy(plaintext: any, entropy: any) {
...
export async function decryptWithEntropy(encrypted: any, entropy: any) {
...
export async function deriveEntropyFromPassword(password: any, salt: any) {
  const keyMaterial = await window.crypto.subtle.importKey(
    'raw',
...
export const encryptData = async (password: any, dataStr: any) => {
  const salt = window.crypto.getRandomValues(new Uint8Array(16));
  const iv = window.crypto.getRandomValues(new Uint8Array(12));
...
export const decryptData = async (password: any, encryptedObj: any) => {
