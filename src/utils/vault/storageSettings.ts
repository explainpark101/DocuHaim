...
export function saveStorageMode(mode: any) {
...
function normalizeWebdavConfig(parsed: any) {
  return {
 ...
 */
export async function saveWebdavConfig(config: any, password?: any) {
 ...
 * @param {string} password
 */
export async function decryptWebdavConfig(password: any) {
  try {
    if (typeof window === 'undefined') return { ...DEFAULT_WEBDAV_CONFIG };
    if (isDesktopApp()) {
      const fromStronghold = await loadDesktopWebdavConfig();
...
export function getAppNameByStorageMode(mode: any) {
