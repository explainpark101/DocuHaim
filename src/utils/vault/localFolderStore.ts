...
const db = new Dexie('s3haim-local-folder') as any;
 ...
 */
export async function saveLocalRootHandle(handle: any) {
  if (!handle) return;
  try {
 ...
 * @returns {Promise<boolean>}
 */
export async function ensureDirectoryReadWritePermission(handle: any) {
