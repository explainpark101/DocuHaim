import { parseChatWithMyselfNoteMeta } from '@/utils/chatWithMyself/format';
 ...
 */
function metaFromHtmlContent(content: any) {
  if (!COMMENT_RE.test(content || '')) return null;
  return parseChatWithMyselfNoteMeta(content);
}

/**
 * @param {import('markdown-it/index').Token} token
 */
function isChatCommentHtmlToken(token: any) {
 ...
 * @param {import('markdown-it/index').Token[]} tokens
 * @param {number} start
 */
function blockquoteEndExclusive(tokens: any, start: any) {
 ...
 * @param {import('markdown-it/index').Token | undefined} inline
 */
function isChatLinkInline(inline: any) {
 ...
 * @param {number} start
 */
function chatLinkParagraphEndExclusive(tokens: any, start: any) {
 ...
 */
export function buildChatSavedNoteCardHtml(meta: any) {
 ...
 */
export function chatSavedNotePlugin(md: any) {
  // Must run after `inline` so link tokens exist inside paragraph inlines.
  md.core.ruler.after('inline', 'chat-saved-note', (state: any) => {
    const { tokens } = state;
