...
function isPgbrHtmlInline(content: any) {
  return /^<pgbr\s*\/?\s*>$/i.test(String(content ?? '').trim());
}

function isPgbrHtmlBlockContent(content: any) {
  return /^<pgbr\s*\/?\s*>$/i.test(String(content ?? '').trim());
}

function createPgbrHtmlInlineToken(state: any) {
...
function countPgbrInInline(inlineToken: any) {
  let n = 0;
...
function isPgbrOnlyParagraph(inlineToken: any) {
...
function createPgbrHtmlBlockToken(state: any) {
...
function transformInlineChildren(state: any, children: any) {
...
export function pageBreakMarkdownItPlugin(md: any) {
  md.core.ruler.push('pgbr-mark', (state: any) => {
  ...
  md.core.ruler.after('pgbr-mark', 'pgbr-unwrap-paragraph', (state: any) => {
    const { tokens } = state;
    const out = [];
    let i = 0;
  ...
  md.core.ruler.after('pgbr-unwrap-paragraph', 'pgbr-normalize-html-block', (state: any) => {
